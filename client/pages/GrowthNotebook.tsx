...
import { Link } from "react-router-dom";
import Header from "@/components/Header";
import { getTodayJalali, gregorianToJalali } from "@/lib/jalali";
...
export default function GrowthNotebook() {
  ...
  useEffect(() => {
    // Get the current child from localStorage
    const childProfile = localStorage.getItem("childProfile");
    if (childProfile) {
      try {
        const child = JSON.parse(childProfile) as ChildData;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-background to-teal-50 pb-8">
      ...
      <div className="bg-white shadow-sm border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <p className="text-slate-600 text-sm">
            {currentChild ? `رشد و پیشرفت ${currentChild.name} را رصد کنید` : "رشد و پیشرفت فرزند خود را رصد کنید"}
          </p>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-md">
            ...
            <div className="space-y-2">
              ...
              {currentData.length === 0 ? (
                <div className="p-6 text-center bg-gradient-to-br from-slate-50 to-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                  <p className="text-slate-600 text-sm mb-2">📊</p>
                  <p className="text-slate-700 font-semibold mb-1">داده‌ای ثبت نشده است</p>
                  <p className="text-slate-500 text-sm">برای شروع، داده‌های فرزند خود را در بخش ثبت داده جدید وارد کنید</p>
                </div>
              ) : (
                currentData.map((entry, idx) => (
                  <div
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-primary text-right"
                />
                <p className="text-xs text-slate-500 mt-1">تاریخ را به صورت شمسی (سال/ماه/روز) وارد کنید</p>
              </div>
